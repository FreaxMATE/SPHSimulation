...
def sph_derivatives(t, y):
    ...
    v_grav_deriv = -constants.gravitational_constant*np.sum(m[None, :, None]*phi_deriv[:, :, None]*x_ij/r_abs[:, :, None], axis=1)
...
scat = axs.scatter(x_init[:, 0], x_init[:, 1], c=rho_init, cmap='viridis', 
                   vmin=rho_min, vmax=rho_max, s=20)
...
anim = animation.FuncAnimation(fig, animate, frames=len(particles.sv_history), 
                              interval=50, blit=False, repeat=True)

# Save as MP4
print("Saving animation as planets_history.mp4...")
anim.save('planets_history_1.mp4', writer='ffmpeg', fps=20, bitrate=1800)
